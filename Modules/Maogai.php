...
class Maogai {
    ...
    public function answer($input) {
        ...
        if (!$this->redis->exists($this->user->weixinID.':maogai:id')) return '系统出错： 缺失毛概标识符';
        $id = $this->redis->get($this->user->weixinID.':maogai:id');
        $pattern = '/([A-Za-z])/';
        ...
        if (!isset($q[++$id])) {
            ...
            $this->redis->del($this->user->weixinID.':maogai:id');
            $this->redis->set($this->user->weixinID.':completed:'.MAOGAI_FILE, 1);
            return '答题完毕，退出答题模式。';
        }
        $this->redis->set($this->user->weixinID.':maogai:id', $id);
    ...
    public function repeat() {
        $id = $this->redis->get($this->user->weixinID.":maogai:id") ?: 0;
        $q = self::getData();
        $result = $q[$id]['type'].'#'.$q[$id]['title']."\r\n";
        foreach ($q[$id]['questions'] as $v) {
            $result .= $v['option'].". ".$v['content']."\r\n";
    ...
    public static function init($weixinID) {
        ...
        $redis->set($weixinID.':maogai:id', 0);
